...
public class TraceGenerator {
	...
	public static ArrayList<ArrayList<String>> generateTrace(StateGraph decider){ 
		...
		ArrayList<ArrayList<StateVertex>> at = decider.getProbablePaths(decider.getDesirable().getDesirable(), ConfigParameters.domain);
		ArrayList<ArrayList<StateVertex>> undesirable = decider.getUndesirablePaths(at, ConfigParameters.domain);
		for(int i=0; i<at.size(); i++){
			ArrayList<StateVertex> list = at.get(i);
			ArrayList<String> trc = new ArrayList<String>();
			for(int j=0; j<list.size()-1; j++){
				ArrayList<ActionEdge> actions = decider.findEdgeForStateTransition(list.get(j), list.get(j+1));
				for (ActionEdge actionEdge : actions) {
					if(edgeInUndesirablePath(actionEdge, undesirable)){ //find trouble action. causing critical state. must be flagged
						...
						if(edgeTriggersCriticalState(actionEdge, decider.getCritical().getCriticalState())){ 
	...
	public static void generateObservationTraceForScenario(){
		for(int trainInstance=0; trainInstance<=22; trainInstance++){
			if(trainInstance==0){
				...
				Decider decider = new Decider(domain, domainFile, desirableStateFile, a_problemFile, a_outputPath, criticalStateFile, a_initFile, a_dotFilePrefix, ConfigParameters.a_dotFile);
